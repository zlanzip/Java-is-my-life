...
public class Collections {
    ...
    /**
     *
     * 使用二分查找在指定List中查找指定元素key。 List中的元
     * 必须是有序的。如果List中有多个key，不能确保哪个key值被找到。
     ...
     * 
     *
     * 如果list不是基于随机访问并且其size>5,会先把List中的复制到数组中， 然后对数组进行混排，再
     * 数组中的元素重新填入List中。 这样做为了避免迭代器大跨度查找元素影响效率
     */
    ...
    /**
     * 
     * 返回集合中的最小元素。前提是其中的元素都是可比的，即实现了Comparable接口 找出一个通用的算法其实不容易，
     * 管它的思想不难。 反正要依次遍历完所有元素，所以直接用了迭代器
     */
    ...
    /**
     * 
     * 返回一个关于指定集合的不可修改的视图。 任何试图修改该视图的
     * 作都将抛出一个UnsupportedOperationException
     * 
    ...
    static class UnmodifiableList<E> extends UnmodifiableCollection<E> implements List<E> {
         ...
         * method was missing in 1.4. 这个，自己看吧...
         */
     ...
     * 动态类型安全视图的一个主要作用是用作debug调试， 因为它能正确反映出出错的位置。 例如：ArrayList<String
     *  strings = new ArrayList<String>(); ArrayList rawList =
     * strings; rawList.add(new Date());
     * add方法并不进行类型检查，所以存入了非String的对象。只有在重新获取该对象 转化为String类型的时候才抛
     * 异常。 而动态类型安全的集合能在add时就会抛出ClassCastException。 这种方法的优点是
     * 误可以在正确的位置被报告
     * 
     *
     */
    ...
    static class CheckedCollection<E> implements Collection<E>, Serializable {
        ...
        /*
         * We don't need locking or volatile, because it's OK if we create several
         * zeroLengthElementArrays, and they're immutable.
         */
    ...
    private static class CheckedMap<K, V> implements Map<K, V>, Serializable {
        ...
        /*
         * We don't need locking or volatile, because it's OK if we create several
         * zeroLengthValueArrays, and they're immutable.
         */
         ...
         * modification of the backing Map via the setValue operation. This class is
        ...
        static class CheckedEntrySet<K, V> implements Set<Map.Entry<K, V>> {
            ...
            public Object[] toArray() {
                ...
                /*
                 * Ensure that we don't get an ArrayStoreException even if s.toArray returns an
                 * array of something other than Object
                 */
             ...
             * nefarious equals function that senses that the equality-candidate is
             * Map.Entry and calls its setValue method.
             */
            public boolean contains(Object o) {
                if (!(o instanceof Map.Entry))
                    return false;
            ...
            /**
             * The next two methods are overridden to protect against an unscrupulous
             * collection whose contains(Object o) method senses when o is a Map.Entry, and
             * calls o.setValue.
             */
            public boolean containsAll(Collection<?> coll) {
                Iterator<?> e = coll.iterator();
            ...
            /**
             * This "wrapper class" serves two purposes: it prevents the client from
             * modifying the backing Map, by short-circuiting the setValue method, and it
             * protects the backing Map against an ill-behaved Map.Entry that attempts to
             * modify another Map Entry when asked to perform an equality check.
             */
    ...
    /**
     * 返回一个比较器，该比较器能使集合按降序排列 例如： Arrays.sort(a, Collections.
     * everseOrder()); 能按字母表相反的顺序排列数组
     * 
     */
